...
def servo_degrees_to_radians(servo_deg: float, joint_name: str) -> float:
    ...
    Args:
        ...
        joint_name: Name of the joint (must be a key in JOINT_CALIBRATION).
...
def radians_to_servo_degrees(joint_rad: float, joint_name: str) -> float:
    ...
    for group_key in group_keys:
        ...
        for joint_name, servo_index in mappings:
            ...
            joint_angles[joint_index] = servo_degrees_to_radians(
                servo_values[servo_index], joint_name
            )
...
def joint_angles_to_servos(
    ...
    for group_key in group_keys:
        ...
        for joint_name, _servo_index in mappings:
            joint_index = config.JOINT_NAMES.index(joint_name)
            servo_deg = radians_to_servo_degrees(joint_angles[joint_index], joint_name)
